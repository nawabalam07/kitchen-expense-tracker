    ...
    <div className="page-wrap">
      ...
      <div className="container">
            ...
            <div style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap' }}>
              {[
                ...
                <div key={s.label} style={{ textAlign: 'right' }}>
                  ...
                  <div className="text-xs text-dim" style={{ textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: '0.2rem' }}>
                    {s.label}
                  </div>
                </div>
              ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <p className="text-sm text-muted">{groupInfo?.group_name}</p>
        </div>
            ...
            <div className="card-scroll">
              {expenses.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p className="text-muted text-sm">Add the first expense and start tracking.</p>
                  <button className="btn btn-saffron" onClick={() => setShowModal(true)}>
                    <PlusIcon /> Add Expense
                  </button>
                </div>
              ) : (
                  ...
                  <div key={exp.expense_id} className="row-item anim-fade-up" style={{ animationDelay: `${i * 0.04}s` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{
                        ...
                        border: `1px solid ${exp.paid_by === user?.user_id ? 'var(--saffron-rim)' : 'var(--rim)'}`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '0.8rem',
                        color: exp.paid_by === user?.user_id ? 'var(--saffron)' : 'var(--t2)',
                      }}>
                        {initials(exp.paid_by_name)}
                      </div>
                      <div>
                        <p style={{ fontWeight: 600, color: 'var(--t1)', fontSize: '0.92rem' }}>{exp.description}</p>
